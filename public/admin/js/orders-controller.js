...
// Client-side filtering + pagination + View button (unchanged) + Edit button (new)
(function () {
  "use strict";
  ...
  function applyFilters() {
    ...
    let arr = [...State.raw];
    if (q) arr = arr.filter(o => [o.id,o.fullName,o.email,o.phone].some(v => (v||"").toString().toLowerCase().includes(q)));
  ...
  function renderRows(){
    ...
    const rows = State.view.slice(start,end).map(o=>{
      ...
      return `<tr data-id="${o.id||""}">
        ...
        <td>${placed}</td>
        <td>
          <button type="button" class="btn-view" data-id="${o.id||""}">View</button>
          <button type="button" class="btn-edit" data-id="${o.id||""}">Edit</button>
        </td>
      </tr>`;
    }).join("");
  ...
  function renderPager(){
    ...
    let html=""; html+=B(1,"«",cur===1); html+=B(Math.max(1,cur-1),"‹",cur===1);
    for(let i=1;i<=pages;i++){ if(i===1||i===pages||Math.abs(i-cur)<=1) html+=B(i,String(i),false,i===cur);
      else if(i===2&&cur>3) html+=`<span class="pg-ellipsis">…</span>`;
      else if(i===pages-1&&cur<pages-2) html+=`<span class="pg-ellipsis">…</span>`; }
    html+=B(Math.min(pages,cur+1),"›",cur===pages); html+=B(pages,"»",cur===pages);
    el.innerHTML=html;
  }

  function wire(){
    const s=$(SEL.search), sa=$(SEL.statusA), sb=$(SEL.statusB), p=$(SEL.pager);
    on(s,"input",debounce(()=>{ State.q=(s.value||"").trim(); State.page=1; applyFilters(); renderRows(); renderPager(); },200));
    const w = el => el && on(el,"change",()=>{ State.status=(el.value||"").trim(); State.page=1; applyFilters(); renderRows(); renderPager(); });
    w(sa); w(sb);
    on(p,"click",(e)=>{ const b=e.target.closest("button.pg-btn"); if(!b) return; const n=parseInt(b.dataset.page,10); if(!Number.isFinite(n)) return; State.page=n; renderRows(); renderPager(); });
    document.addEventListener("click",(e)=>{
      const b=e.target.closest(".btn-view"); if(b){ const id=b.getAttribute("data-id"); window.dispatchEvent(new CustomEvent("orders:view",{detail:{id}})); }
    });
  }
  ...
  function auto(){
    ...
    wire(); fetchOnce().catch(err=>console.error("[Orders] load failed:", err));
  }

  if(document.readyState==="loading") document.addEventListener("DOMContentLoaded", auto); else auto();
  window.addEventListener("admin:partial-loaded",(e)=>{ if(e?.detail?.name==="orders") auto(); });

  // simple built-in 'View' modal (unchanged)
  function ensureViewModal(){
    let dlg=document.getElementById("orderViewDialog"); if(dlg) return dlg;
  ...
  function fmtKES(n){ try{return new Intl.NumberFormat(undefined,{style:"currency",currency:"KES",maximumFractionDigits:0}).format(n||0);}catch{return "KSH "+(n||0).toLocaleString();}}
  window.addEventListener("orders:view",(e)=>{
    const id=e.detail?.id; const o=State.raw.find(x=>String(x.id)===String(id)); if(!o) return;
    const dlg=ensureViewModal(); const c=dlg.querySelector(".content");
    const items=(o.items||[]).map(it=>`<tr><td>${it.sku||"—"}</td><td>${it.name||"—"}</td><td>${it.qty||1}</td><td>${fmtKES(it.price||0)}</td></tr>`).join("");
