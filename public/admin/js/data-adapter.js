...
(function (global) {
  ...
  function fmtKES(v){
    try { return new Intl.NumberFormat(undefined,{style:"currency",currency:"KES",maximumFractionDigits:0}).format(v); }
  ...
  function normOrder(o={}){
    const id = o.id ?? o.orderId ?? o.orderNumber ?? o.number ?? o.reference ?? null;
    ...
    const createdAt = toISO(o.createdAt ?? o.timestamp ?? o.placedAt ?? o.date) ?? null;
    const total = typeof o.total === "number" ? o.total : parseKES(o.total ?? o.amount ?? o.grandTotal);
    const items = Array.isArray(o.items) ? o.items.map(it => ({
      sku: it.sku ?? it.code ?? it.id ?? null,
      name: it.name ?? it.title ?? "",
      qty: toInt(it.qty ?? it.quantity ?? 1, 1),
      price: typeof it.price === "number" ? it.price : parseKES(it.price ?? it.unitPrice ?? 0),
    })) : [];
    return { id, fullName: name, phone, email, status, total, createdAt, items, raw:o };
  }

  function normUser(u={}){
    return { id: u.id ?? u.userId ?? null, name: u.name ?? u.fullName ?? "", email: u.email ?? "", phone: u.phone ?? "", type: u.type ?? u.role ?? "Customer", status: u.status ?? "Active", createdAt: toISO(u.createdAt ?? u.created ?? u.date) ?? null, raw:u };
  }

  function normItem(i={}){
    const price = typeof i.price === "number" ? i.price : parseKES(i.price ?? i.unitPrice);
    return { id: i.id ?? i.sku ?? i.code ?? null, name: i.name ?? i.title ?? "", price, stock: toInt(i.stock ?? i.qty ?? i.quantity ?? 0, 0), category: i.category ?? i.cat ?? "", createdAt: toISO(i.createdAt ?? i.date) ?? null, raw:i };
  }
  ...
  async function getOrders({ q="", status="", page=1, per=DEFAULTS.per }={}){
    ...
    if(page)params.set("page",String(page)); if(per)params.set("per",String(per));
    const data = await fetchJSON(`${DEFAULTS.apiBase}/api/orders?${params.toString()}`);
  ...
  async function getUsers({ type="", page=1, per=DEFAULTS.per }={}){
    ...
    if(page)params.set("page",String(page)); if(per)params.set("per",String(per));
    const data = await fetchJSON(`${DEFAULTS.apiBase}/api/users?${params.toString()}`).catch(()=>({users:[]}));
  ...
  async function getItems({ q="", page=1, per=DEFAULTS.per }={}){
    ...
    if(page)params.set("page",String(page)); if(per)params.set("per",String(per));
    const data = await fetchJSON(`${DEFAULTS.apiBase}/api/items?${params.toString()}`).catch(()=>({items:[]}));
  ...
  async function patchOrder(id, { status, driverId=null, notes="" }){
    const data = await fetchJSON(`${DEFAULTS.apiBase}/api/admin/orders/${encodeURIComponent(id)}`, {
      method: "PATCH",
