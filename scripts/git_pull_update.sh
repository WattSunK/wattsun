#!/bin/sh
# =====================================================
# WattSun NAS Git Pull (self-healing + non-disruptive)
# -----------------------------------------------------
# - Verifies remote origin
# - Auto-stashes any local/runtime diffs before pull
# - Fetches latest changes
# - Fast-forwards current branch (no hard reset)
# - Logs old/new SHAs and changed files
# - Does NOT stop/start services or run npm installs
# =====================================================
...
# Optional: auto-accept new GitHub host key
...
# --- 🧩 Auto-stash runtime diffs before pull ---
echo "[step] Auto-stashing any local/runtime diffs..."
git stash push --include-untracked -m "auto-stash before NAS pull $(date +%Y%m%d_%H%M%S)" >/dev/null 2>&1 || true
echo "[info] Local changes safely stashed (if any)."
...
if git merge --ff-only "origin/$BRANCH"; then
  ...
  echo "[hint] Resolve manually or run:"
