#!/bin/bash
# ============================================================
# ⚡ promote_to_qa.sh — Optimized Dev → QA Promotion (Incremental Rsync + Auto-Verify)
# ============================================================
set -e
...
echo -e "${CYAN}============================================================"
echo -e "🚀  WattSun — Promote Dev → QA (Incremental Rsync + Auto-Verify)"
echo -e "============================================================${NC}"
...
# --- Step 2️⃣: Incremental Rsync (local) ---
...
RSYNC_CMD="sudo rsync -aHAX --inplace --partial --size-only --no-whole-file \
  ...
  --exclude='qa/data/' --exclude='qa/logs/' --exclude='qa/run/' \
  --exclude='qa/scripts/' --exclude='.git/' --exclude='node_modules/' \
  \"$ROOT/\" \"$QA_ROOT/\""

echo -e "${CYAN}Running: $RSYNC_CMD${NC}"
eval $RSYNC_CMD || { echo -e "${YELLOW}⚠️ Rsync exited abnormally — retrying once...${NC}"; sleep 3; eval $RSYNC_CMD || { echo -e "${RED}❌ Rsync failed after retry. Aborting.${NC}"; exit 1; }; }
...
if [ ! -f "$DEV_DB" ]; then
  echo -e "${RED}❌ DEV DB not found at $DEV_DB${NC}"; exit 1
fi
...
if [ -f "$QA_DB" ]; then
  ...
  sudo cp "$QA_DB" "$BACKUP_FILE"; sudo chown 53Bret:users "$BACKUP_FILE"
fi

sudo cp "$DEV_DB" "$QA_DB"
sudo chown 53Bret:users "$QA_DB"; sudo chmod 664 "$QA_DB"
...
if [ ! -x "$ROOT/scripts/loyalty_reset.sh" ]; then
  echo -e "${RED}❌ loyalty_reset.sh not found or not executable.${NC}"; exit 1
fi
echo -e "${YELLOW}🧹 Running loyalty_reset.sh qa (targeting new QA DB)...${NC}"
DB_OVERRIDE="$QA_DB"; export DB_OVERRIDE
...
export NODE_ENV=qa
export DB_PATH_USERS="$QA_DB"; export SQLITE_DB="$QA_DB"
export DB_PATH_INVENTORY="$QA_ROOT/data/qa/inventory.qa.db"; export SQLITE_MAIN="$QA_DB"
...
# --- Step 7️⃣: Final Combined Verification ---
echo -e "${YELLOW}🧾 Final Verification (Backend + DB) ...${NC}"
HEALTH_MSG=$(curl -s http://127.0.0.1:3000/api/health || echo "unreachable")
USER_COUNT=$(sqlite3 "$QA_DB" "SELECT COUNT(*) FROM users;" 2>/dev/null || echo "0")
echo -e "${CYAN}------------------------------------------------------------${NC}"
echo -e "🌐 Backend Health: ${HEALTH_MSG}"
echo -e "🗄️  QA DB Users:   ${USER_COUNT}"
echo -e "${CYAN}------------------------------------------------------------${NC}"
