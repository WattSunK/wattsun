/**
 * routes/checkout.js
 * WattSun – Checkout Route (Crash-Safe Final)
 */
...
router.post("/", (req, res) => {
  ...
  const totalCents = items.reduce((s, i) => s + (parseFloat(i.price) || 0) * (i.quantity || 1), 0) * 100;
        ...
        stmt.finalize((e2) => {
          ...
          db.run(
            ...
            (metaErr) => {
              ...
              // 4️⃣ Notifications (fixed placeholder count)
              try {
                const nq = db.prepare(
                  `INSERT OR IGNORE INTO notifications_queue
                   (kind, user_id, email, payload, status, dedupe_key)
                   VALUES (?, ?, ?, ?, ?, ?)`
                );
                ...
                console.warn("[checkout] notifications insert warning:", notifyErr);
              }
